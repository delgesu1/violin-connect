...
type MasterRepertoireWithSource = MasterRepertoire & { _source?: 'api' | 'cache' | 'mock' };
...
const PieceDetailsPage = () => {
  ...
  // Add debugging for student history
  useEffect(() => {
    console.log("Student history data:", studentHistory);
  }, [studentHistory]);
  ...
  const transformPiece = (masterPiece: MasterRepertoire) => {
    if (!masterPiece) {
      console.error('No piece data provided to transformPiece');
      return {
        id: pieceId || 'unknown',
        masterPieceId: pieceId || 'unknown',
    ...
    return {
      id: masterPiece.id || pieceId || 'unknown',
      masterPieceId: masterPiece.id || pieceId || 'unknown', // Self-reference for master pieces
      startDate: masterPiece.started_date || new Date().toISOString().split('T')[0],
      status: 'current' as const,
      title: masterPiece.title || 'Unknown Piece',
      composer: masterPiece.composer || 'Unknown Composer',
      notes: masterPiece.notes || undefined,
      _source: (masterPiece as any)._source || 'mock'
    };
  };
  ...
  if (pieceLoading || filesLoading || linksLoading || historyLoading) {
    return (
      <div className="container mx-auto px-4 py-6">
        <PageHeader title="Loading Piece Details" />
  ...
  const safePiece = piece || {} as MasterRepertoireWithSource;
  const transformedPiece = transformPiece(safePiece);
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6 pb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b">
          <div className="space-y-1.5">
            <h1 className="text-3xl font-bold">{safePiece.title || 'Unknown Piece'}</h1>
            <p className="text-xl text-muted-foreground">{safePiece.composer || 'Unknown Composer'}</p>
            {(safePiece as any)._source && (
              <Badge variant={(safePiece as any)._source === 'api' ? 'outline' : 'secondary'}>
                Source: {(safePiece as any)._source}
              </Badge>
            )}
          </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {file._source && (
                      <Badge 
                        variant={
                          file._source === 'api' ? 'outline' : 
                          file._source === 'cache' ? 'secondary' : 
                          'destructive'
                        }
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {link._source && (
                      <Badge 
                        variant={
                          link._source === 'api' ? 'outline' : 
                          link._source === 'cache' ? 'secondary' : 
                          'destructive'
                        }
                        className="opacity-0 group-hover:opacity-100 transition-opacity ml-2 text-xs"
                      >
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <Avatar className="h-8 w-8">
                              <AvatarImage src={student.student_photo_url} alt={student.student_name || 'Student'} />
                              <AvatarFallback className="bg-primary/10 text-primary text-xs">
                                {student.student_name ? student.student_name.split(' ').map(n => n[0]).join('') : 'S'}
                              </AvatarFallback>
                            </Avatar>
                            <div>
                              <div className="font-medium">{student.student_name || 'Unknown Student'}</div>
                              <div className="flex items-center text-xs text-muted-foreground">
                                <Clock className="h-3 w-3 inline mr-1" />
                                <span>Started {student.start_date || 'Unknown date'}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            {student._source && (
                              <Badge variant={student._source === 'api' ? 'outline' : 'secondary'} className="text-xs">
                                {student._source}
                              </Badge>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center text-xs text-muted-foreground">
                                ...
                                <span>Completed {student.end_date || 'Unknown date'}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
