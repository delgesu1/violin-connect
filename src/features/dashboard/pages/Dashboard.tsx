...
import { useAllLessons } from '@/features/calendar/hooks';
...
const Dashboard = () => {
  ...
  const getStudentName = (lesson: any) => {
    ...
    // Fallback for student_name field
  ...
  const getStudentAvatar = (lesson: any) => {
    if (lesson.student?.avatar_url) {
      return lesson.student.avatar_url;
    }
    
    if (lesson.student_avatar) {
      return lesson.student_avatar;
    }
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <Avatar className="h-10 w-10 border">
                      ...
                      <AvatarFallback className="bg-primary/10 text-primary">
                        {getStudentName(lesson).substring(0, 2)}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <h3 className="font-medium text-gray-900">{getStudentName(lesson)}</h3>
